...
app.put('*', setFileMeta, setDirMeta, wrap(createFile), (req, res, next) => {
    if(req.stat) return res.status(405).send('files exists')
    res.end()
})


app.post('*', setFileMeta, setDirMeta, wrap(updateFile), (req, res, next) => {
    if(!req.stat) return res.status(405).send('files exists')
    if(req.isDir) return res.status(405).send('this is directory')
...
function setFileMeta(req, res, next) {
    console.log("inside setFileMeta function.....")
...
function* deleteResources(req, res, next) {
    console.log("inside deleteResources()....")
    if (req.stat.isDirectory()) {
